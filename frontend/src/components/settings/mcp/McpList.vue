<template>
  <div class="space-y-4">
    ...
    <div v-if="servers.length > 0" class="flex items-center px-4 py-2 bg-slate-50 rounded-lg border border-slate-200">
        <input 
            type="checkbox" 
            :checked="allSelected"
        ...
        >
        <span class="ml-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Servidores ({{ servers.length }})</span>
    </div>

    <div v-if="servers.length === 0" class="text-center py-8 text-slate-500 bg-slate-50 rounded-xl border border-dashed border-slate-300">
    ...
    <div v-for="server in servers" :key="server.id" 
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-2 text-xs text-slate-500">
            <span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">{{ server.type }}</span>
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="$emit('edit', server)" class="p-2 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title="Editar">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
        </button>
        <button @click="$emit('delete', server)" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
