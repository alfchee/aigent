  ...
  <div class="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <span v-if="saveOk" class="text-xs text-emerald-600 font-medium animate-pulse"
          >Guardado</span
        >
        <button
          type="button"
    ...
    <main class="flex-1 p-4 md:p-8 max-w-5xl mx-auto w-full space-y-6">
      <div v-if="loading" class="text-center py-12 text-slate-500">
        <div
          class="animate-spin h-8 w-8 border-4 border-sky-500 border-t-transparent rounded-full mx-auto mb-2"
        ></div>
      ...
      <div v-else class="space-y-6">
        <div
          v-if="error"
          class="text-sm text-red-600 border border-red-200 bg-red-50 rounded-xl p-4 flex items-start gap-2"
        >
          <svg
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm min-h-[400px]">
          ...
          <div v-if="activeTab === 'general'" class="p-6 space-y-8">
            <section class="space-y-4">
              ...
              <div class="bg-sky-50 border border-sky-100 rounded-lg p-4 flex items-start gap-3">
                ...
                <div class="ml-0">
                  <label for="autoEscalate" class="font-medium text-slate-900 text-sm"
                    >Activar Inteligencia Adaptativa</label
                  >
                  <p class="text-slate-600 text-xs mt-1">
                    Si el modelo principal falla o encuentra errores técnicos, el sistema
                    reintentará automáticamente con el modelo Inteligente.
                  </p>
                </div>
              </div>
            </section>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <span
                    ...
                    "
                    >{{ providers.google ? 'ACTIVO' : 'INACTIVO' }}</span
                  >
                </div>
                <div
                  class="p-3 rounded-lg border flex items-center justify-between"
                  ...
                  <span
                    ...
                    :class="
                      providers.openai
                        ...
                        : 'bg-slate-200 text-slate-500'
                    "
                    >{{ providers.openai ? 'ACTIVO' : 'INACTIVO' }}</span
                  >
                </div>
                <div
                  class="p-3 rounded-lg border flex items-center justify-between"
                  ...
                  <span
                    ...
                    :class="
                      providers.brave
                        ...
                        : 'bg-slate-200 text-slate-500'
                    "
                    >{{ providers.brave ? 'ACTIVO' : 'INACTIVO' }}</span
                  >
                </div>
              </div>
            </section>
          ...
          <div v-if="activeTab === 'roles'" class="p-6 space-y-8">
            <section class="space-y-4">
              ...
              <div
                v-if="emergencyMode"
                class="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3 animate-pulse"
              >
                <svg
                ...
                <div>
                  ...
                  <p class="text-xs text-red-700 mt-1">
                    Todos los roles "Pro" han sido degradados a modelos "Flash" para conservar
                    tokens y evitar límites de tasa. La configuración de roles a continuación será
                    ignorada temporalmente a favor de modelos eficientes.
                  </p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="p-4 border border-slate-200 rounded-xl bg-slate-50 space-y-3">
                  <div class="flex items-center justify-between">
                    <label class="block text-sm font-bold text-slate-800"
                      >Manos (Code/File Worker)</label
                    >
                    <span
                      class="text-xs px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 font-medium"
                      >Ejecución</span
                    >
                  </div>
                  <select
                ...
                <div class="p-4 border border-slate-200 rounded-xl bg-slate-50 space-y-3">
                  <div class="flex items-center justify-between">
                    <label class="block text-sm font-bold text-slate-800"
                      >Reloj (Scheduled Worker)</label
                    >
                    <span
                      class="text-xs px-2 py-0.5 rounded bg-amber-100 text-amber-700 font-medium"
                      >Monitoreo</span
                    >
                  </div>
                  <select
                ...
                <div class="p-4 border border-slate-200 rounded-xl bg-slate-50 space-y-3">
                  <div class="flex items-center justify-between">
                    ...
                    <span
                      class="text-xs px-2 py-0.5 rounded bg-purple-100 text-purple-700 font-medium"
                      >Audio</span
                    >
                  </div>
                  <select
                ...
                <div class="p-4 border border-slate-200 rounded-xl bg-slate-50 space-y-3">
                  <div class="flex items-center justify-between">
                    <label class="block text-sm font-bold text-slate-800"
                      >Imagen (Image Generator)</label
                    >
                    <span class="text-xs px-2 py-0.5 rounded bg-rose-100 text-rose-700 font-medium"
                      >Imagen</span
                    >
                  </div>
                  <select
          ...
          <div v-if="activeTab === 'personality'" class="p-6 space-y-6">
            <div class="flex items-start gap-4">
              <div class="flex-1 space-y-4">
                <div class="space-y-1">
                  ...
                  <p class="text-sm text-slate-500">
                    Define la identidad, el tono y las reglas fundamentales de comportamiento del
                    agente.
                  </p>
                </div>
                <textarea
              ...
              <div
                class="w-64 bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 hidden md:block"
              >
          ...
          <div v-if="activeTab === 'limits'" class="p-6 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                />
                <p class="text-xs text-slate-500">
                  Tiempo máximo que el agente esperará una respuesta de una herramienta.
                </p>
              </div>
              ...
              <div class="space-y-2">
                ...
                />
                <p class="text-xs text-slate-500">
                  Número máximo de enlaces que el agente recuperará por búsqueda.
                </p>
              </div>
              ...
              <div class="space-y-2">
                ...
                />
                <p class="text-xs text-slate-500">
                  Cuántas veces reintentar una operación fallida antes de rendirse.
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'advanced'" class="p-6 h-full flex flex-col">
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <p class="text-sm text-slate-500">
                  Configuración cruda del orquestador.
                  <span class="text-amber-600 font-medium">Uso avanzado.</span>
                </p>
              </div>
              <div
                v-if="jsonError"
                class="text-xs text-red-600 font-bold bg-red-50 px-2 py-1 rounded"
              >
                {{ jsonError }}
              </div>
              <div
                v-else
                class="text-xs text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded"
              >
                JSON Válido
              </div>
            </div>
